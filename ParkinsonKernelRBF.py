...
train_file_path = 'data/continuous/parkinson_data-VOICE-features-train1.csv'
test_file_path = 'data/continuous/parkinson_data-VOICE-features-test.csv'

# Read the train and test datasets
train_data = pd.read_csv(train_file_path)
test_data = pd.read_csv(test_file_path)

# Convert 'MDVP_Shimmer_dB' to numeric in both train and test datasets, setting non-numeric values to NaN
train_data['MDVP_Shimmer_dB'] = pd.to_numeric(train_data['MDVP_Shimmer_dB'], errors='coerce')
test_data['MDVP_Shimmer_dB'] = pd.to_numeric(test_data['MDVP_Shimmer_dB'], errors='coerce')

# Drop rows where 'MDVP_Shimmer_dB' has NaN values in both train and test datasets
train_data = train_data.dropna(subset=['MDVP_Shimmer_dB'])
test_data = test_data.dropna(subset=['MDVP_Shimmer_dB'])
...
X_train = train_data[['MDVP_Shimmer_dB']].head(train_sample_size).values
X_test = test_data[['MDVP_Shimmer_dB']].head(test_sample_size).values
...
plt.title("RBF Kernel Vectors (MDVP_Shimmer_dB)", fontsize=16)
